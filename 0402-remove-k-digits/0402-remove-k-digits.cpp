...
public:
    string removeKdigits(string num, int k) {
        ...
        {
            char curr = num[i];
            //if(!st.empty() && curr < st.top() && k>=1)
            while(ans.size() != 0 && curr < ans.back() && k>=1)
            {
                ans.pop_back();
                k--;
            }
...
When we encounter the next element we check if it is smaller than the element present in the stack top if so pop the element from
